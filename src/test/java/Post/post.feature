Feature: Post an album
  ...
  * url 'https://jsonplaceholder.typicode.com/users/1/albums'
  ...
  Scenario: Post an album
    ...
    When method post
    Then status 201
    And request [{"userId": 1,"id": 22,"title": "Esta es una nuevo ingreso"}]
    And request [{"userId": 1,"id": 22,"title": "Esta es una nuevo ingreso actualizado"}]
    And print response


    Scenario: Crear un nuevo album
      Given request
       """
    {
      "userId": 1,
    ...
    """
      When method post
      Then status 201
      And match response contains { title: 'Nuevo álbum' }
      And print response
